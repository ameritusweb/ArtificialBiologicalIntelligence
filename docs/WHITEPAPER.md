...
**The depth_reached observation (generation 29, seed 42).** At generation 29, depth_reached activated at partial correlation 0.284 — becoming the dominant thinking channel in a single generation. Metacognition and conflation had both activated at generations 27-28. However, a replication run (seed 99, 40 generations) did not reproduce the depth_reached activation despite both prerequisites being present from generation 17 onward. The depth_reached observation is a single-seed finding and should not be treated as a replicated result. The prerequisite pattern (metacognition and conflation emerging before any depth response is possible) replicates across both seeds.

**The convergence result (replicated).** The conflation receptor was added to the genome based on theoretical reasoning. The organism evolved it independently in both seeds — generation 27-28 in seed 42, generation 17 in seed 99 — under selection pressure from stochastic hidden confounders. Two independent paths — human theoretical reasoning and evolutionary selection — converged on the same receptor because the receptor detects something real about the structure of the problem. The convergence is replicated; the specific claim that conflation was load-bearing for depth_reached is based on one observation.

**What MCTS contributes and what it doesn't.** Metacognition and conflation emerge from environmental complexity alone — they appeared in both seeds without MCTS being required. What MCTS added is a new class of observable internal signals that make thinking quality visible to selection pressure. Without MCTS, thinking depth is an internal process with no fitness consequence. With MCTS, the 6 thinking channels externalize the thinking process into the observation vector. MCTS doesn't make the organism smarter — it makes the organism's smartness selectable. This claim is supported; the specific depth_reached activation is not replicated.

**Negative control result.** Three fake receptors (for structure the environment provably doesn't contain) were inserted into the battery. After null calibration, one false positive survived — magnetoreception, which detected a real correlation between efference copy channels and action (a structural confound, not a magnetic field). This establishes an empirical false positive rate of ~0.5% per test, or ~0.4 expected false positives among 77 discovered receptors. Tests correlating action with action-derived observation channels (efference copy, last_actions) have a structural confound that action-shuffled null cannot remove.
...
- **Cross-environment transfer**: evolved organisms (gen 29 checkpoint) vs naive organisms in same novel environment. 71 shared receptors, 30 transfer-only, 27 naive-only. The trunk converges; the canopy diverges based on evolutionary history. The trunk is universal. The canopy is biography.
