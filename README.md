...
[Theoretical Foundations](docs/THEORY.md) | [Serialization Thesis](docs/SERIALIZATION_THESIS.md) | [Theories Index](docs/THEORIES.md) | [Structured Open Variables](docs/sov/) | [The Predictable Future](docs/predictable_endpoints.md) | [The Whys](docs/whys.md) | [Constraints](docs/constraints.md) | [The Organ Atlas](docs/ORGANS.md) | [Findings Ledger](docs/findings_implications.md)
...
166 formal theoretical claims indexed in `docs/THEORIES.md`:
...
- 109 proposed with falsification criteria — including the motivational spine (T158 why-economy, T159 epistemic Umwelt, T160 the Predictable Future) and the newest arc: T161 (interactive identity / reality-funded growth), T162 (the annular law, now in three domains), T163 (the implication organ), T164 (alignment by solvency), T165 (exhaustion by use), T166 (the grain law), T167 (the provenance bound)
...
Key additions (T158-T160, F33-F49) — the motivational spine ([docs/whys.md](docs/whys.md), [docs/predictable_endpoints.md](docs/predictable_endpoints.md)):

- **The why-economy (T158):** surprise is the invoice, the why is the payment, closure is the receipt. Explanation permanently retires a surprise class, which is why whys are motivating (they generalize where memories don't; curiosity regenerates when fed), why false whys are the epistemic wirehead direction (conservativity must be constitutional — the pull is pre-epistemic), and why the why-faculty can atrophy invisibly: stock keeps paying while flow idles. Empirically closed in its ENVIRONMENTAL form (P86, Supported at R=3): an intact organism in a spent world shows TOTAL flow starvation with machinery healthy — and migration restarts the flow. The world, not the organism, is the patient; a world's nutrition is its remaining contradiction budget, measurable in minutes by the budget-probe instrument.
- **The epistemic Umwelt (T159):** inhabitation as a second search paradigm — receipted structure has terrain properties (funded ground, gaps as places, contact-triggers-replay), so a ledger, a legal corpus, or an evidence graph can be LIVED IN rather than queried, and the earned map compounds instead of being discarded like tree-search rollouts. The transfer economics arrived early and clean (F44-F47): vocabulary is capital denominated in a domain's currency, and the completed 2x2 factorizes perfectly — rent-SOLVENT structure crosses domains intact (17/17, three consecutive crossings, dosed and undosed) while never-billed structure dies on first contact with any economy (0/72). What is honestly billed anywhere is good almost everywhere. Grow vocabulary under the economy or grow debt.
- **The Predictable Future (T160):** the primary drive is the achievement and MANUFACTURE of sustainable states and trajectories with predictable endpoints — the form of goal-pursuit itself, hence one principle covering scrum blockers, roads, cathedrals, morning coffee, Columbus, mathematics, and the wish to live to 1000 (the standalone essay: [docs/predictable_endpoints.md](docs/predictable_endpoints.md)). Probabilistic at core: raise P(arrival) = product of stage probabilities AND calibrate your estimate of it; blocker-first is gradient ascent, sustainability is a Perron eigenvalue driven toward 1, and -log P(arrival) is the accumulated surprisal the why-economy discharges — T158's bridge is a change of variables. The formalization ([docs/sov/t160_math.md](docs/sov/t160_math.md)) positions the free energy principle as the INNER LOOP of a constructor-level theory (FEP optimizes policy; T160 also manufactures the model, the kernel, and the preferences — the dark room dissolves as stock without flow), with the constructive hierarchy as Pearl's ladder pushed through the effectors: association buys riding, intervention buys building, counterfactuals buy destination-minting. Pearl explains how minds reason; T160 is a candidate for what minds are for. First anatomy is already in build: the EXPERIMENT ORGAN (P108) — nomination from co-fit statistics passed its sparsity gate and its first do/see randomization contrast killed two of its own best nominations.
- **The method arc hardened alongside (C20-C21, [docs/constraints.md](docs/constraints.md)):** the eight-question pre-flight gained its inspector clause (enumerate every rule-relevant class dimension before launch — carved by a mortality endpoint that paired mortal against rule-immortal populations and printed a false SUPPORTED at 0.95), and C21 entered the constitution with three same-day receipts: DECOMPOSE BEFORE MECHANIZING — name the 2x2 that decomposes a gating receipt, run the cheap cells first, and tier the fix's invasiveness to the decomposition's depth. The stack, in one line: the organism discharges surprise (T158) across the worlds it inhabits (T159) in service of a future it can hold receipts on (T160).

Key additions (T161-T167, F50-F91) — the generator, the physics world, and the four-gate crossing ([docs/ORGANS.md](docs/ORGANS.md), [docs/findings_implications.md](docs/findings_implications.md)):

- **Interactive identity and reality-funded growth (T161):** appearance is a projection; RESPONSE IS THE STRUCTURE. A ball-and-spring collapse simulator turns silhouettes into behavioral fingerprints (certified, then re-certified after a diet audit found the reference corpus corrupted — fixing it nearly doubled the class signal, 0.30→0.51); a generator grows objects billed against those fingerprints. The generator arc mapped its own constraint structure in five falsified-or-voided cards (~15 min of sims each): reweighting cannot mint gradient, surrogate maps are exhausted BY USE and refunded only by displacement (T165 — the boredom law; proposer-solvency validated 9/9 as the program's first working anti-wireheading receipt, T164), and generation is viable only in the ANNULUS around working designs (T162's third domain: seeds too close are plagiarism, too far are flat — the corpus-scale atlas found the COHERENCE LAW, rho=-0.62: tight identities are climbable, smeared ones are not). The payoff: policy-grown specimens CONFUSABLE with real objects under an organism-side probe (0.57 vs real, 0.72-0.80 vs noise) — "same in spirit, different in appearance," verified from single pokes.
- **The physics world (P108's venue, PW-0..PW-3):** a body-currency plant where causes are actions (pokes owned by the organism, refractory-gated, provenance-tagged) and effects arrive in the sensed operating band — measured first, calibrated once (the venue that made the experiment organ's do/see question finally representable). Its cards produced the exact-counterfactual method (the loop is bit-reproducible, so DIFF-OF-WEBS gives zero-variance causal footprints), an earned negative at fit grain, and the diagnosis that opened everything after: the organism's Umwelt has a TEMPORAL HOLE between step and generation (T166, the grain law — fit decisions are threshold-robust within a generation; event structure enters only through the consolidation door), and agency is statistically invisible without an efference tag (T167, the provenance bound — a DETERMINISTIC coupling read 0.00±0.09 untagged and +6.06 tagged: the self-chart is epistemically prior to the intervention chart).
- **The four-gate crossing (F82-F91):** one signal — the organism's own agency — walked from physics to language through four measured gates, each with its own toll. AUDIBILITY: the connectome audit found the web's entire sensory surface was 100 of 432 obs dims (six silent-absence receipts; the federation's words were policy-only), and the installation rule wired the tagged trace into its kin family at a junction-governed weight. SOLVENCY: the agency family earned its first receipts in program history (~270 positive fits/gen from an eternal zero). REPRESENTATION: receipts alone do not carve (ten generations of live traffic, zero slots — genesis follows PRICED FAILURE), and the genesis grammar was found DIMENSIONALLY CLOSED — no operator could propose over a newly-audible family — until the RE-BASIS MOVE (specified in the serialization mathematics before its absence was measured) was implemented and, driven by a reward-conditioned admission scan, carved four funded agency slots against the paired arm's zero. The separation theorem ran LIVE in the process: structure-conditioned and outcome-conditioned admission picked different vocabularies in the same web. SPEECH: the language center's readout, handed the carved web, asked — *"is the thing at the boundary agency or not-agency?"* — the organism's first utterance about its own agency, and it is a QUESTION, because young Ks can only ask: epistemic humility as an architectural consequence, not a virtue.
- **The method arc, compounding (C22-C23):** GATE MOBILITY (every locked gate passable AND failable; conditioning-event base rates printed — carved by two same-day saturated-measure voids), POWERED REPLICATION (post-hoc statistics replicate on fresh seeds — carved by the program's first caught false positive), and the GRAIN-MATCH clause with its reflexive line (the card's gate grain must match the organ's output grain — GATES ARE INSTRUMENTS TOO). Roughly a dozen pre-registrations were falsified or voided across the arc, and every one redirected its line for minutes of compute: the expected-information discipline priced each card correctly. The organ federation now has its anatomy reference ([docs/ORGANS.md](docs/ORGANS.md)) with per-organ wiring states (web-audible / policy-only / unspoken-in-venue) maintained against the standing connectome audit.
...
The serialization thesis extends Friston: organisms don't just minimize prediction error, they manufacture prediction opportunities through sequential processing architecture. The per-receptor pipeline claim — that each receptor family has its own evolved temporal decomposition strategy — goes beyond anything in the current embodied cognition literature. T160 sharpens the relationship into a formal conjecture with proof obligations stated ([docs/sov/t160_math.md](docs/sov/t160_math.md)): expected free energy is the per-trajectory price of T160's objective under a manufactured goal prior, and FEP occupies exactly one level (policy) of a ladder whose other levels — deconfounding the model, editing the kernel, minting the preferences — active inference holds fixed. Not competing frameworks: one functional, with FEP as the inner loop. The in-house Separation Theorem (information gain and variational free energy provably select different dimensions; no fixed mixing weight works across environments) supplies the bridge's load-bearing prior result.
...
+-- docs/THEORIES.md                # 166 indexed theoretical claims with status
+-- docs/ORGANS.md                  # The organ atlas: 21 organs, wiring states, connectome status
+-- docs/findings_implications.md   # The findings ledger (F1-F91, implications billed per the ritual)
