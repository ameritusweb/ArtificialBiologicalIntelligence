"""ACCELERATION v4 — full protocol + RECEIPTED PAIR (card locked at
launch 2026-08-13; completes the three-factor decomposition).

DIAGNOSIS (C21, third cell): v1/v2 (stripped loop, pair 97100/97101)
gave 1 and 0 closures; v3 (FULL protocol, same pair) gave 0 — so the
protocol factor alone does not explain the silence. The remaining
factor is the PAIR: closure is world-selective (F30), and every
closure receipt at these horizons (F28, p86_v4's P1 closures in 1-4
gens) was earned on pair B (97000/97001). v4 = v3 with pair B. If the
cold arm closes here at receipted rates, the silence decomposes as
PAIR-SELECTivity and the warm-vs-cold comparison is finally live; if
even pair B is silent, something in the two-arm harness still differs
from p86_v4 and the diff hunt goes to the code, not the venue.
Solvent-survival caveat carried from v3: replay_scans SPLITS slots
(parent -> children), so the survival curve conflates differentiation
with death; reported descriptively, never billed as mortality without
split/eviction decomposition. Original v3 header follows.
...
def main():
    t0 = time.time()
    ro.CHURN_MIN = 2
    print('=== ACCELERATION v4: full replay protocol ===')
    ...
    cold_web = ConstraintWeb(eigen_coder=ReceptorEigenCoder(),
                             debug_level=0, ledger_id='ACC4_COLD')
    ...
    if wc + cc < CLOSURE_FLOOR:
        ...
        verdict = ('PARTIAL: warm %d/g%s vs cold %d/g%s'
                   % (wc, wl, cc, cl))
    print('\nACCELERATION v4 VERDICT: %s' % verdict)
