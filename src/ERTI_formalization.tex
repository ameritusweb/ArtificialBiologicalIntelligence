...
%% ============================================================
\section{The Umwelt: Topology on Observation Space}

\begin{definition}[Receptor-Generated Topology]
Each receptor $r \in \mathcal{R}_t$ has a detection set $C_r \subseteq \mathbb{R}^n$.
The collection $\{C_r : r \in \mathcal{R}_t\}$ forms a \textbf{subbasis} for a topology
$\mathcal{T}_t$ on the observation space $\mathbb{R}^n$. This topology defines which
observations the organism can distinguish.
\end{definition}

\begin{definition}[Receptor Equivalence and Umwelt]
Two observations $\mathbf{x}, \mathbf{y} \in \mathbb{R}^n$ are \textbf{receptor-equivalent}
($\mathbf{x} \sim_{\mathcal{R}_t} \mathbf{y}$) iff no receptor separates them:
\[
\mathbf{x} \sim_{\mathcal{R}_t} \mathbf{y} \iff \forall r \in \mathcal{R}_t : \mathbf{x} \in C_r \Leftrightarrow \mathbf{y} \in C_r
\]
The \textbf{Umwelt} of the organism is the quotient space $\mathbb{R}^n / \sim_{\mathcal{R}_t}$.
The organism does not live in $\mathbb{R}^n$. It lives in its Umwelt --- the set of
equivalence classes under receptor separation. This formalizes von Uexk\"ull (1909):
each organism inhabits a subjective world defined by what its receptors can separate.
\end{definition}

\begin{theorem}[Topological Refinement]
If $\mathcal{R}_t \subset \mathcal{R}_{t+1}$ (receptors are added but not lost), then
$\mathcal{T}_{t+1}$ strictly refines $\mathcal{T}_t$: every open set in $\mathcal{T}_t$
is open in $\mathcal{T}_{t+1}$, and $\mathcal{T}_{t+1}$ contains additional open sets.
The Umwelt gains resolution --- observations that were previously indistinguishable may
become separable.
\end{theorem}

\begin{proof}
If $C_r \in \mathcal{T}_t$ for all $r \in \mathcal{R}_t$, then $C_r \in \mathcal{T}_{t+1}$
since $\mathcal{R}_t \subset \mathcal{R}_{t+1}$. The new receptor $r' \in \mathcal{R}_{t+1}
\setminus \mathcal{R}_t$ contributes $C_{r'} \in \mathcal{T}_{t+1}$ which is not necessarily
in $\mathcal{T}_t$, so the refinement is strict unless $C_{r'}$ is already generated by
existing subbasis elements.
\end{proof}

\begin{definition}[Common Refinement]
Given two organisms with topologies $\mathcal{R}^{(1)}$ and $\mathcal{R}^{(2)}$, their
\textbf{common refinement} is the topology generated by $\mathcal{R}^{(1)} \cap \mathcal{R}^{(2)}$.
Two organisms can communicate about exactly those distinctions that survive in the common
refinement. A concept is \textbf{untranslatable} from organism 1 to organism 2 when the
open set corresponding to it is in $\mathcal{T}^{(1)}$ but cannot be built from the subbasis
of $\mathcal{T}^{(1)} \cap \mathcal{T}^{(2)}$.
\end{definition}

\begin{observation}[Trunk as Common Ground]
Across 80 generations (seed 42), 18 receptors are invariant --- present in every generation.
These generate the coarsest topology shared by all organisms in the evolutionary lineage.
In cross-environment transfer (two organisms with different evolutionary histories in the
same environment), 134 of 191 total receptors are shared, 30 are transfer-only, 27 are
naive-only. The shared set is the common refinement; the organism-specific sets are
untranslatable in each direction.
\end{observation}

\begin{theorem}[Self-Model Incompleteness]
No RTDS can contain a receptor $r^*$ that is \textbf{topology-complete}: a receptor whose
detection set separates every pair of observations that any receptor in $\mathcal{R}_t$
separates. For $|\mathcal{R}_t| > 1$, the number of distinguishable states in the full
topology (up to $2^{k_t}$) exceeds what a single scalar-valued receptor can encode.
Therefore self-models are necessarily incomplete.
\end{theorem}

\begin{proof}
A scalar $\phi_{r^*}(\mathbf{x}) \in [0, \infty)$ can encode at most a countable partition
of $\mathbb{R}^n$ (via its level sets). But the topology $\mathcal{T}_t$ generated by $k_t > 1$
subbasis elements can distinguish up to $2^{k_t}$ equivalence classes. For $k_t > 1$, a single
scalar cannot faithfully represent which of the $2^{k_t}$ classes a state belongs to. Any
encoding loses information about the joint activation pattern. More precisely: define
$D = \{\mathbf{x} : \phi_{r^*}(\mathbf{x}) \leq \theta_{r^*}\}$. $D$ is a legitimate
detection condition. For $r^*$ to be topology-complete, it must separate $D$ from its
complement, but $D$ is defined by $r^*$'s own activation --- a fixed point equation with
no non-trivial solution.
\end{proof}

\begin{remark}
This is the diagonal argument applied to detection systems. It mirrors G\"odel's
incompleteness: a sufficiently rich detection system cannot detect all truths about itself.
The RTDS evolves --- it can add receptors, closing specific blind spots while creating new
ones at the new boundary of self-reference. The organism can always learn more about itself.
It can never learn everything.
\end{remark}
