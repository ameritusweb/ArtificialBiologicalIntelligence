"""ACCELERATION v3 — full replay protocol (card locked at launch
2026-08-13; closes the harness-factor decomposition of v1/v2's silence).
...
def main():
    t0 = time.time()
    ro.CHURN_MIN = 2
    print('=== ACCELERATION v3: full replay protocol ===')
    ...
    cold_web = ConstraintWeb(eigen_coder=ReceptorEigenCoder(),
                             debug_level=0, ledger_id='ACC3_COLD')
    ...
    if wc + cc < CLOSURE_FLOOR:
        ...
        verdict = ('PARTIAL: warm %d/g%s vs cold %d/g%s'
                   % (wc, wl, cc, cl))
    print('\nACCELERATION v3 VERDICT: %s' % verdict)
